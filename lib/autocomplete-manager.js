...
class AutocompleteManager {
  ...
  handleCommands () {
    return this.subscriptions.add(atom.commands.add('atom-text-editor', {
      'autocomplete-plus:activate': (event) => {
        if (!this.isValidContext()) return event.abortKeyBinding()
  ...
  /// Returns true if the last cursor is next to a non-whitespace.
  isValidContext() {
    const cursor = this.editor.getLastCursor();
    const position = cursor.getBufferPosition();
    if (position.column === 0) return false;
    const char = this.editor.getBuffer().getTextInRange([[position.row, position.column-1], position]);
    return char.match(this.getWordCharacterRegex());
  }
