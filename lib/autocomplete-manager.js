...
export default class AutocompleteManager {
  ...
  handleEvents () {
    ...
    if (atom.textEditors != null) {
      this.subscriptions.add(atom.textEditors.observe(editor => {
        ...
        return view.addEventListener('focus', (element) => {
          this.updateCurrentEditor(editor)
        })
      }
      ))
    ...
    this.subscriptions.add(atom.config.observe('autocomplete-plus.suppressActivationForEditorClasses', value => {
      ...
      for (let i = 0; i < value.length; i++) {
        let selector = value[i]
        let classes = (selector.trim().split('.').filter((className) => className.trim()).map((className) => className.trim()))
  ...
  findSuggestions (activatedManually) {
    ...
    let prefix = this.getPrefix(this.editor, bufferPosition)
  ...
  getSuggestionsFromProviders (options) {
    ...
    let providers = this.providerManager.applicableProviders(options.editor, options.scopeDescriptor)

    let providerPromises = []
    providers.forEach(provider => {
      let apiVersion = this.providerManager.apiVersionForProvider(provider)
      let apiIs20 = semver.satisfies(apiVersion, '>=2.0.0')
      ...
      return providerPromises.push(Promise.resolve(getSuggestions(upgradedOptions)).then(providerSuggestions => {
        ...
        for (let i = 0; i < providerSuggestions.length; i++) {
          let suggestion = providerSuggestions[i]
        ...
        if (hasEmpty) {
          let res = []
          for (let s of providerSuggestions) {
            if (s.snippet || s.text) {
              res.push(s)
            }
          }
  ...
  filterSuggestions (suggestions, {prefix}) {
    ...
    for (let i = 0; i < suggestions.length; i++) {
      ...
      let text = (suggestion.snippet || suggestion.text)
      let suggestionPrefix = suggestion.replacementPrefix != null ? suggestion.replacementPrefix : prefix
      let prefixIsEmpty = !suggestionPrefix || suggestionPrefix === ' '
      let firstCharIsMatch = !prefixIsEmpty && suggestionPrefix[0].toLowerCase() === text[0].toLowerCase()
...
See https://github.com/atom/autocomplete-plus/wiki/Provider-API`
  ...
  getUniqueSuggestions (suggestions) {
    ...
    for (let i = 0; i < suggestions.length; i++) {
      let suggestion = suggestions[i]
      let val = suggestion.text + suggestion.snippet
      if (!seen[val]) {
  ...
  getPrefix (editor, bufferPosition) {
    ...
    let prefix = this.prefixRegex.exec(line)
    if (!prefix || !prefix[2]) {
      return ''
    }
    return prefix[2]
  ...
  confirm (suggestion) {
    ...
    let apiVersion = this.providerManager.apiVersionForProvider(suggestion.provider)
    let apiIs20 = semver.satisfies(apiVersion, '>=2.0.0')
    let triggerPosition = this.editor.getLastCursor().getBufferPosition()
  ...
  replaceTextWithMatch (suggestion) {
    ...
    return this.editor.transact(() => {
      for (let i = 0; i < cursors.length; i++) {
        ...
        let endPosition = cursor.getBufferPosition()
        let beginningPosition = [endPosition.row, endPosition.column - suggestion.replacementPrefix.length]

        if (this.editor.getTextInBufferRange([beginningPosition, endPosition]) === suggestion.replacementPrefix) {
          let suffix = this.consumeSuffix ? this.getSuffix(this.editor, endPosition, suggestion) : ''
  ...
  getSuffix (editor, bufferPosition, suggestion) {
    ...
    let endPosition = [bufferPosition.row, bufferPosition.column + suffix.length]
    let endOfLineText = editor.getTextInBufferRange([bufferPosition, endPosition])
    let nonWordCharacters = new Set(atom.config.get('editor.nonWordCharacters').split(''))
    while (suffix) {
  ...
  isCurrentFileBlackListed () {
    ...
    let fileName = path.basename((left = this.buffer.getPath()) != null ? left : '')
    for (let i = 0; i < this.fileBlacklist.length; i++) {
      let blacklistGlob = this.fileBlacklist[i]
  ...
  toggleActivationForBufferChange ({newText, newRange, oldText, oldRange}) {
    ...
    if (this.autoActivationEnabled || this.suggestionList.isActive()) {
      if (newText.length > 0) {
        ...
        if (newText.length === 2) {
          for (let pair of this.bracketMatcherPairs) {
            if (newText === pair) {
              this.shouldActivate = true
            }
          }
  ...
  showOrHideSuggestionListForBufferChanges ({changes}) {
    let lastCursorPosition = this.editor.getLastCursor().getBufferPosition()
    let changeOccurredNearLastCursor = changes.some(function ({start, newExtent}) {
      let newRange = new Range(start, start.traverse(newExtent))
  ...
  showOrHideSuggestionListForBufferChange ({newText, newRange, oldText, oldRange}) {
    ...
    let cursorPositions = this.editor.getCursorBufferPositions()
  ...
  shouldSuppressActivationForEditorClasses () {
    for (let i = 0; i < this.suppressForClasses.length; i++) {
      let classNames = this.suppressForClasses[i]
      let containsCount = 0
