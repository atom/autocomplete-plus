...
export default class FuzzyProvider {
  constructor () {
    ...
    let builtinProviderBlacklist = atom.config.get('autocomplete-plus.builtinProviderBlacklist')
  ...
  getSuggestions ({editor, prefix, scopeDescriptor}) {
    ...
    let suggestions = this.findSuggestionsForWord(prefix, scopeDescriptor)
  ...
  bufferWillChange ({oldRange}) {
    let oldLines = this.editor.getTextInBufferRange([[oldRange.start.row, 0], [oldRange.end.row, Infinity]])
    return this.removeWordsForText(oldLines)
  }

  bufferDidChange ({newRange}) {
    let newLines = this.editor.getTextInBufferRange([[newRange.start.row, 0], [newRange.end.row, Infinity]])
  ...
  addWordsForText (text) {
    ...
    let matches = text.match(this.wordRegex)
  ...
  getCompletionsForCursorScope (scopeDescriptor) {
    let completions = this.settingsForScopeDescriptor(scopeDescriptor, 'editor.completions')
    let seen = {}
    let resultCompletions = []
    for (let i = 0; i < completions.length; i++) {
      let {value} = completions[i]
      if (Array.isArray(value)) {
        for (let j = 0; j < value.length; j++) {
          let completion = value[j]
          if (!seen[completion]) {
