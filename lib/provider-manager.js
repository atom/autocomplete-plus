...
export default class ProviderManager {
  ...
  filterProvidersByScopeDescriptor (providers, scopeDescriptor) {
    let scopeChain = scopeChainForScopeDescriptor(scopeDescriptor)
    ...
    for (let i = 0; i < providers.length; i++) {
      let providerMetadata = providers[i]
    ...
    if (disableDefaultProvider) {
      let index = matchingProviders.indexOf(defaultProviderMetadata)
  ...
  sortProviders (providers, scopeDescriptor) {
    let scopeChain = scopeChainForScopeDescriptor(scopeDescriptor)
    return stableSort(providers, function (providerA, providerB) {
      let priorityA = providerA.provider.suggestionPriority != null ? providerA.provider.suggestionPriority : 1
      let priorityB = providerB.provider.suggestionPriority != null ? providerB.provider.suggestionPriority : 1
      let difference = priorityB - priorityA
      if (difference === 0) {
        let specificityA = providerA.getSpecificity(scopeChain)
        let specificityB = providerB.getSpecificity(scopeChain)
  ...
  metadataForProvider (provider) {
    for (let i = 0; i < this.providers.length; i++) {
      let providerMetadata = this.providers[i]
  ...
  registerProvider (provider, apiVersion = '3.0.0') {
    ...
    let apiIs200 = semver.satisfies(apiVersion, '>=2.0.0')
    let apiIs300 = semver.satisfies(apiVersion, '>=3.0.0')
...
const scopeChainForScopeDescriptor = function (scopeDescriptor) {
  // TODO: most of this is temp code to understand #308
  let type = typeof scopeDescriptor
  ...
  if (type === 'string') {
    ...
    let scopeChain = scopeDescriptor.getScopeChain()
    if ((scopeChain != null) && (scopeChain.replace == null)) {
      let json = JSON.stringify(scopeDescriptor)
