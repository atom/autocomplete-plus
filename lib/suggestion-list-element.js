...
let ItemTemplate = `<span class="icon-container"></span>
...
let ListTemplate = `<div class="suggestion-list-scroller">
...
let IconTemplate = '<i class="icon"></i>'

let DefaultSuggestionTypeIconHTML = {
...
class SuggestionListElement extends HTMLElement {
  ...
  moveSelectionPageUp () {
    let newIndex = Math.max(0, this.selectedIndex - this.maxVisibleSuggestions)
    if (this.selectedIndex !== newIndex) { return this.setSelectedIndex(newIndex) }
  }

  moveSelectionPageDown () {
    let itemsLength = this.visibleItems().length
    let newIndex = Math.min(itemsLength - 1, this.selectedIndex + this.maxVisibleSuggestions)
    if (this.selectedIndex !== newIndex) { return this.setSelectedIndex(newIndex) }
  }
  ...
  moveSelectionToBottom () {
    let newIndex = this.visibleItems().length - 1
    if (this.selectedIndex !== newIndex) { return this.setSelectedIndex(newIndex) }
  }
  ...
  renderItems () {
    ...
    let items = (left = this.visibleItems()) != null ? left : []
    let longestDesc = 0
    let longestDescIndex = null
    for (let index = 0; index < items.length; index++) {
      let item = items[index]
      this.renderItem(item, index)
      let descLength = this.descriptionLength(item)
  ...
  scrollSelectedItemIntoView () {
    let { scrollTop } = this.scroller
    let selectedItemTop = this.selectedLi.offsetTop
    ...
    let { itemHeight } = this.uiProps
    let scrollerHeight = (this.maxVisibleSuggestions * itemHeight) + this.uiProps.paddingHeight
  ...
  renderItem ({iconHTML, type, snippet, text, displayText, className, replacementPrefix, leftLabel, leftLabelHTML, rightLabel, rightLabelHTML}, index) {
    ...
    let typeIconContainer = li.querySelector('.icon-container')
    typeIconContainer.innerHTML = ''

    let sanitizedType = escapeHtml(isString(type) ? type : '')
    let sanitizedIconHTML = isString(iconHTML) ? iconHTML : undefined
    let defaultLetterIconHTML = sanitizedType ? `<span class=\"icon-letter\">${sanitizedType[0]}</span>` : ''
    let defaultIconHTML = DefaultSuggestionTypeIconHTML[sanitizedType] != null ? DefaultSuggestionTypeIconHTML[sanitizedType] : defaultLetterIconHTML
    if ((sanitizedIconHTML || defaultIconHTML) && iconHTML !== false) {
      typeIconContainer.innerHTML = IconTemplate
      let typeIcon = typeIconContainer.childNodes[0]
      typeIcon.innerHTML = sanitizedIconHTML != null ? sanitizedIconHTML : defaultIconHTML
      if (type) { this.addClassToElement(typeIcon, type) }
    }

    let wordSpan = li.querySelector('.word')
    wordSpan.innerHTML = this.getDisplayHTML(text, snippet, displayText, replacementPrefix)

    let leftLabelSpan = li.querySelector('.left-label')
    ...
    let rightLabelSpan = li.querySelector('.right-label')
  ...
  getDisplayHTML (text, snippet, displayText, replacementPrefix) {
    ...
    if (typeof displayText === 'string') {
      ...
      let snippets = this.snippetParser.findSnippets(replacementText)
      replacementText = this.removeSnippetsFromText(snippets, replacementText)
      var snippetIndices = this.findSnippetIndices(snippets)
    }
    let characterMatchIndices = this.findCharacterMatchIndices(replacementText, replacementPrefix)
  ...
  removeSnippetsFromText (snippets, text) {
    ...
    let index = 0
    let result = ''
    for (let {snippetStart, snippetEnd, body} of snippets) {
  ...
  findSnippetIndices (snippets) {
    ...
    for (let {snippetStart, snippetEnd, body} of snippets) {
      ...
      let startIndex = snippetStart - offsetAccumulator
      let endIndex = (startIndex + bodyLength) - 1
  ...
  findCharacterMatchIndices (text, replacementPrefix) {
    ...
    if (this.useAlternateScoring) {
      let matchIndices = fuzzaldrinPlus.match(text, replacementPrefix)
