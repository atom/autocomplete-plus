...
module.exports = class SuggestionListElement {
  constructor(model) {
    ...
    this.descriptionContent = this.element.querySelector(
      '.suggestion-description-content'
    );
    this.descriptionMoreLink = this.element.querySelector(
    ...
    this.subscriptions.add(
      this.model.onDidChangeItems(this.itemsChanged.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidChangeItem(this.itemChanged.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectNext(this.moveSelectionDown.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectPrevious(this.moveSelectionUp.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectPageUp(this.moveSelectionPageUp.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectPageDown(this.moveSelectionPageDown.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectTop(this.moveSelectionToTop.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidSelectBottom(this.moveSelectionToBottom.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidConfirmSelection(this.confirmSelection.bind(this))
    );
    this.subscriptions.add(
      this.model.onDidconfirmSelectionIfNonDefault(
        this.confirmSelectionIfNonDefault.bind(this)
      )
    );
    ...
    this.subscriptions.add(
      atom.config.observe(
        'autocomplete-plus.suggestionListFollows',
    ...
    this.subscriptions.add(
      atom.config.observe(
        'autocomplete-plus.maxVisibleSuggestions',
  ...
  setDescriptionMoreLink(item) {
    if (
      item.descriptionMoreURL != null &&
      item.descriptionMoreURL.length != null
    ) {
  ...
  moveSelectionPageUp() {
    const newIndex = Math.max(
      0,
      this.selectedIndex - this.maxVisibleSuggestions
    );
  ...
  renderItems() {
    ...
    for (
      let index = 0;
      index < Math.min(items.length, this.maxVisibleSuggestions + 1);
      index++
    ) {
    ...
    return atom.views.updateDocument(
      this.returnItemsToPool.bind(this, items.length)
    );
  }
  ...
  scrollSelectedItemIntoView() {
    ...
    const scrollerHeight =
      this.maxVisibleSuggestions * itemHeight + this.uiProps.paddingHeight;
  ...
  renderItem(
    ...
    li.querySelector('.word-container').replaceChild(
      wordSpan,
      li.querySelector('.word')
    );
  ...
  getDisplayFragment(
    ...
    if (!characterMatchIndices) {
      characterMatchIndices = this.findCharacterMatchIndices(
        replacementText,
        replacementPrefix
      );
    } else {
    ...
    for (let index = 0; index < replacementText.length; index++) {
      if (
        snippetIndices &&
        (snippetIndices[index] === SnippetStart ||
          snippetIndices[index] === SnippetStartAndEnd)
      ) {
      ...
      if (characterMatchIndices && characterMatchIndices[index]) {
        nonMatchChars = addTatweelToNonMatchedStringEndBeforeMatchedString(
          nonMatchChars,
          replacementText,
          index,
          isolatedChars
        ...
        s.textContent = '';
        addTatweelToMatchedStringBeginingAfterNonMatchedString(
          index,
          replacementText,
          isolatedChars,
          s
        ...
        for (
          ;
          index < replacementText.length && characterMatchIndices[index];
          index++
        )
          s.textContent += replacementText[index];
        index--;

        addTatweelToMatchedStringEndBeforeNonMatchedString(
          index,
          replacementText,
          isolatedChars,
          s
        ...
        nonMatchChars = addTatweelToNonMatchedStringBeginingAfterMatchedString(
          index,
          replacementText,
          characterMatchIndices,
          isolatedChars,
      ...
      if (
        snippetIndices &&
        (snippetIndices[index] === SnippetEnd ||
          snippetIndices[index] === SnippetStartAndEnd)
      ) {
        appendNonMatchChars(workingEl, nonMatchChars);
        nonMatchChars = '';
...
function addTatweelToNonMatchedStringBeginingAfterMatchedString(
  index,
  replacementText,
  characterMatchIndices,
  isolatedChars,
  ...
  if (
    ...
    characterMatchIndices &&
    characterMatchIndices[index - 1] &&
    isArabicLetter(replacementText[index - 1]) &&
    !isolatedChars.includes(replacementText[index - 1])
  )
    nonMatchChars += 'ـ';
  return nonMatchChars;
}

function addTatweelToMatchedStringEndBeforeNonMatchedString(
  index,
  replacementText,
  isolatedChars,
  s
  ...
  if (
    index + 1 < replacementText.length &&
    replacementText[index + 1] !== ' ' &&
    isArabicLetter(replacementText[index]) &&
    !isolatedChars.includes(replacementText[index])
  )
    s.textContent += 'ـ';
}

function addTatweelToNonMatchedStringEndBeforeMatchedString(
  nonMatchChars,
  replacementText,
  index,
  isolatedChars
  ...
  if (
    nonMatchChars.length > 0 &&
    replacementText[index] !== ' ' &&
    isArabicLetter(nonMatchChars[nonMatchChars.length - 1]) &&
    !isolatedChars.includes(nonMatchChars[nonMatchChars.length - 1])
  )
    nonMatchChars += 'ـ';
  return nonMatchChars;
}

function addTatweelToMatchedStringBeginingAfterNonMatchedString(
  index,
  replacementText,
  isolatedChars,
  s
...
function isArabicLetter(char) {
  if (isArabicNumber(char)) return false
  return /[\u0600-\u06FF\u0750-\u077F\uFB50-\uFC3F\uFE70-\uFEFC]/.test(char);
}

function isArabicNumber(char) {
  return /[\u0660-\u0661-\u0662-\u0663-\u0664-\u0665-\u0666-\u0667-\u0668-\u0669]/.test(char)
}
