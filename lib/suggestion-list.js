...
export default class SuggestionList {
  ...
  addBindings (editor) {
    ...
    let completionKey = atom.config.get('autocomplete-plus.confirmCompletion') || ''
    ...
    let useCoreMovementCommands = atom.config.get('autocomplete-plus.useCoreMovementCommands')
    let commandNamespace = useCoreMovementCommands ? 'core' : 'autocomplete-plus'
  ...
  */
  ...
  showAtBeginningOfPrefix (editor, prefix, followRawPrefix = false) {
    ...
    if (followRawPrefix || this.wordPrefixRegex.test(prefix)) { bufferPosition = bufferPosition.translate([0, -prefix.length]) }
    ...
    if (this.activeEditor === editor) {
      ...
      this.displayBufferPosition = bufferPosition
      let marker = this.suggestionMarker = editor.markBufferRange([bufferPosition, bufferPosition])
