...
export default class SymbolStore {
  ...
  symbolsForConfig (config, buffers, prefix, wordUnderCursor, cursorBufferRow, numberOfCursors) {
    ...
    for (let bufferLines of this.linesForBuffers(buffers)) {
      ...
      for (let bufferLine of bufferLines) {
        let symbolsForLetter = bufferLine[0].get(firstLetter) || EMPTY_ARRAY
        for (let symbol of symbolsForLetter) {
  ...
  recomputeSymbolsForEditorInBufferRange (editor, oldRange, newRange) {
    let newLines = []
    ...
    this.linesForBuffer(editor.getBuffer()).splice(oldRange.start.row, oldRange.getExtent().row + 1, ...newLines)
  }
